        ...
        fetch('../js/data/conversations.json')
            ...
            .then(data => {
                ...
                data.forEach(conv => {
                    ...
                    div.id = conv.id;
                    div.innerHTML = `
                        <img src="${conv.avatar}" alt="${escapeHtml(conv.partnerName)}" class="avatar">
                        <div style="flex:1;">
                            <div class="flex justify-between items-center" style="margin-bottom: 0.25rem;">
                                <strong class="text-sm font-bold">${escapeHtml(conv.partnerName)}</strong>
                                <span class="text-xs text-text-lighter" style="font-size: 0.75rem;">${escapeHtml(conv.timestamp)}</span>
                            </div>
                            <p class="text-sm text-text-light" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px;">
                                ${escapeHtml(conv.lastMessage)}</p>
                        </div>
                        ${badgeHtml}
                    `;
                                ...
                                const html = `
                                    <div class="message-bubble message-${msg.type}">
                                        ...
                                        <div class="text-right" style="font-size: 0.7rem; margin-top: 0.25rem; ${msg.type === 'sent' ? 'color: rgba(255,255,255,0.8);' : 'color: var(--color-text-lighter);'}">
                                            ${escapeHtml(msg.time)} ${msg.type === 'sent' ? '&check;&check;' : ''}
                                        </div>
                                    </div>
                                `;
                                chatContainer.insertAdjacentHTML('beforeend', html);
