...
function renderArtworkDetail(artwork) {
    ...
    if (categoryEl) categoryEl.textContent = artwork.category.charAt(0).toUpperCase() + artwork.category.slice(1);
    if (priceEl) priceEl.textContent = artwork.price || '';
    ...
    if (descEl) {
        ...
        if (paragraphs.length > 0) {
            ...
            descEl.innerHTML = `<p>${escapeHtml(artwork.description)}</p>`;
        }
    }
